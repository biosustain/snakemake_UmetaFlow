...
def filter(feature_files, out_featurexml):
    ...
    df["feature_ids"] = [
        [handle.getUniqueId() for handle in cf.getFeatureList()] for cf in consensus_map
    ]
    df = df.reset_index()
    ...
    def remove_blank_features(blanks, samples, cutoff):
        ...
        avg_samples = samples.drop(columns=["feature_ids"])
        avg_samples = avg_samples.mean(axis=1, skipna=False)
    ...
    for fmap in feature_maps:
        if (
            os.path.basename(fmap.getMetaValue("spectra_data")[0].decode())[7:]
            not in blank_files
        ):
