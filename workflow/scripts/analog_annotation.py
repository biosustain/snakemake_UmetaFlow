...
def ms2query_annotations(matrix, ms2query_csv, annotated):
    ...
    Analog_matrix = Analog_matrix.rename(
        columns={
    ...
    for i, id in zip(Matrix.index, Matrix["id"]):
        ...
        for (
            ...
            Analog_matrix["ms2query_model_prediction"],
            Analog_matrix["analog_compound_name"],
            Analog_matrix["smiles"],
            Analog_matrix["feature_id"],
            Analog_matrix["precursor_mz_query_spectrum"],
            Analog_matrix["npc_superclass_results"],
            Analog_matrix["cf_superclass"],
            Analog_matrix["cf_class"],
        ):
            if analog_id == id:
                hit1 = f"{score}"
                hit2 = f"{name}"
