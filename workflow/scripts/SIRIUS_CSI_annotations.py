...
def sirius_csi_annotations(matrix, sirius, csi, annotated):
    ...
    for tsv in input_formulas:
        ...
        if "opt_global_rank" in df.columns:
            ...
            df = df.drop(columns=df.filter(regex=rf"Score").columns)
            df = df.drop(columns=df.filter(regex=rf"opt").columns)
            df = df.reset_index()
    ...
    DF_features["feature_ids"] = [
        ids[1:-1].split(",") for ids in DF_features["feature_ids"]
    ]
    ...
    for i, id, sirius in zip(
        DF_features.index, DF_features["feature_ids"], DF_features["SIRIUS_predictions"]
    ):
        hits1 = []
        hits2 = []
        hits3 = []
        hits4 = []
        for name, formula, smiles, Pred_id, db in zip(
            DF_CSI["name"],
            DF_CSI["formulas"],
            DF_CSI["smiles"],
            DF_CSI["featureId"],
