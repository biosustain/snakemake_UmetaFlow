...
def lib_training(input_LIB, ms2library, ion_mode):
    ...
    cleaned_library_spectra = clean_normalize_and_split_annotated_spectra(
        library_spectra, ion_mode_to_keep=ion_mode
    )[0]
    library_creator = LibraryFilesCreator(
        cleaned_library_spectra,
        output_directory=join(
            "results", "Interim", "annotations", "ms2query"
        ),  # For instance "data/library_data/all_GNPS_positive_mode_"
        ms2ds_model_file_name=join(
            "resources", "ms2query", "ms2ds_model_GNPS_15_12_2021.hdf5"
        ),  # The file location of the ms2ds model
        s2v_model_file_name=join(
            "resources", "ms2query", "spec2vec_model_GNPS_15_12_2021.model"
        ),
    )  # The file location of the s2v model
    library_creator.create_all_library_files()

    ms2_library_directory = (
        ms2library  # Specify the directory containing all the library and model files
    )
