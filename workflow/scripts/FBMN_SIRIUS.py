...
def integration(input_matrix, input_mgf, input_graphml, output_graphml):
    ...
    DF_SIRIUS["opt_global_featureId"] = DF_SIRIUS["opt_global_featureId"].str.replace(
        r"id_", ""
    )
    ...
    input_structures = glob.glob(
        os.path.join("results", "SiriusCSI", "structures_*.tsv")
    )
    DF_CSI = pd.DataFrame()
    list_of_df = []
    ...
    DF_CSI["opt_global_featureId"] = DF_CSI["opt_global_featureId"].str.replace(
        r"id_", ""
    )
    ...
    for result in DF_SIRIUS.to_dict(orient="records"):
        ...
        if scan in G:
            ...
            G.nodes[scan]["sirius:explainedIntensity"] = result[
                "opt_global_explainedIntensity"
            ]
            G.nodes[scan]["sirius:explainedPeaks"] = result["opt_global_explainedPeaks"]
    ...
    for result in DF_CSI.to_dict(orient="records"):
        ...
        if scan in G:
            ...
            G.nodes[scan]["csifingerid:Confidence_Score"] = result[
